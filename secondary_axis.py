...
"""
def get_z_tick_strings(z_plot):

    z_ticks = Vector{AbstractString}(undef, length(z_plot))

    for i in range(1,length(z_plot)):
...
"""
def get_z_secondary_axis(ax, c=cosmology(), z_plot=[15, 4, 2, 1, 0.5, 0.3, 0.1, 0],color="k"):
...
def get_cr_energy_axis(ax, CR_type::String="p",color="k")
